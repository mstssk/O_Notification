package jp.mstssk.o_notification;

class NotifyUtils {
    static final String CHANNEL_ID_1ST = "CHANNEL_ID_1ST";
    static final String CHANNEL_ID_2ND = "CHANNEL_ID_2ND";
    static final String CHANNEL_ID_CONFIG = "CHANNEL_ID_CONFIG";
    static final String CHANNEL_GROUP_ID_FOO = "CHANNEL_GROUP_ID_FOO";
    static final String CHANNEL_ID_3RD_WITH_GROUP_FOO = "CHANNEL_ID_3RD_WITH_GROUP_FOO";
    static final String CHANNEL_ID_4th_WITH_GROUP_FOO = "CHANNEL_ID_4th_WITH_GROUP_FOO";
    static final String CHANNEL_ID_TIMEOUT = "CHANNEL_ID_TIMEOUT";

}
